...
export default function TrainDetail({ onBack }: TrainDetailProps) {
  ...
  useEffect(() => {
    const fetchQuickCheck = async () => {
      try {
        const res = await fetch(`${BASE_URL}/api/traffic/trains`);
        const data = await res.json();
        
        // 過濾出民雄與嘉義站的資料進行分組
        const stations = ["Minxiong", "Chiayi"];
        const grouped = stations.map(s => ({
          station: `${s} Station`,
          trains: data.filter((t: any) => t.stationName === s || t.stationName.includes(s))
        }));
        setQuickCheckData(grouped);
      } catch (error) {
        console.error("Failed to fetch train board:", error);
      }
    };
    fetchQuickCheck();
  }, []);
  ...
  const handleSearch = async () => {
    setLoading(true);
    try {
      // 依照 API 文件 query string 格式，這裡假設後端 search 接受 station 參數
      ...
      const res = await fetch(`${BASE_URL}/api/traffic/trains/search?${query}`);
      const data = await res.json();
