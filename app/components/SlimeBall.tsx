...
export default function SlimeBall() {
  ...
  useEffect(() => {
    ...
    const maxMove = 25;
    const limitedX = Math.max(-maxMove, Math.min(maxMove, dx / 8));
    const limitedY = Math.max(-maxMove, Math.min(maxMove, dy / 8));
  ...
  return (
    ...
    >
      <motion.svg
        ...
        className="w-full h-full drop-shadow-[0_15px_30px_rgba(255,100,100,0.35)]"
      >
        <defs>
          <linearGradient id="slimeGradient" x1="0%" y1="100%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#FFC837" />
            <stop offset="40%" stopColor="#FF8008" />
      ...
      <div className="absolute inset-0 flex items-center justify-center gap-7 pointer-events-none">
        <motion.div
          ...
          style={{ x: eyeX, y: eyeY }}
          animate={{
            width: isBlinking ? 16 : 14,
            height: isBlinking ? 4 : 22,
          }}
