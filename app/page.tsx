...
export default function Home() {
  ...
  // 2. 修改：調整導覽列順序，將 Campus 移到第一位，Announcement 順延
  const navItems = [
    { name: "Campus", icon: MapPin, id: "campus" }, // 移到最前面
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "essentials":
        return <Essentials subTab={essentialsSubTab} />;
        ...
        <div className="max-w-6xl mx-auto mb-12">
          <div className="flex flex-wrap items-center justify-between gap-y-6">
            {navItems.map((item, idx) => {
              ...
              // --- 1. Campus 的下拉選單邏輯 (現在移到了這裡) ---
              if (item.id === "campus") {
                return (
                  <div key={idx} className="relative group z-30">
                    <button
                      onClick={() => setActiveTab(item.id)}
                      className={`relative z-20 flex flex-row items-center gap-3 cursor-pointer p-2 rounded-lg transition-all duration-200 ${
                        isActive
                          ...
                          {campusSubItems.map((subItem) => (
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                setActiveTab("campus");
                                setCampusSubTab(subItem);
                              }}
                              className={`px-4 py-3 text-left text-xs font-bold transition-colors hover:bg-emerald-50 hover:text-emerald-600 ${
                                activeTab === "campus" &&
                                campusSubTab === subItem
                                          ...
                                          (childItem) => (
                                            <button
                                              ...
                                              onClick={(e) => {
                                                ...
                                                setEssentialsSubTab(childItem);
                                              }}
                                              className={`px-4 py-3 text-left text-xs font-bold transition-colors hover:bg-emerald-50 hover:text-emerald-600 ${
                                                activeTab === "essentials" &&
                                                essentialsSubTab === childItem
                                                  ? "text-emerald-600 bg-emerald-50/50"
                                                  : "text-gray-500"
                                              }`}
                                            >
                            ...
                            return (
                              <button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setActiveTab("essentials");
                                  setEssentialsSubTab(subItem);
                                }}
                                className={`px-4 py-3 text-left text-xs font-bold transition-colors hover:bg-emerald-50 hover:text-emerald-600 ${
                                  activeTab === "essentials" &&
                                  essentialsSubTab === subItem
                                    ? "text-emerald-600 bg-emerald-50/50"
                                    : "text-gray-500"
                                }`}
                              >
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end pointer-events-none">
            ...
            >
              <div className="bg-gradient-to-r from-emerald-500 to-teal-600 p-4 flex justify-between items-center text-white">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-sm">CCU Assistant</h3>
                    <p className="text-[10px] text-emerald-50 opacity-90">
                      Ask me anything
                    </p>
                  </div>
                </div>
              ...
              <div className="flex-1 bg-gray-50 p-4 overflow-y-auto">
                <div className="flex flex-col gap-3">
                  <div className="self-start bg-white px-4 py-3 rounded-2xl rounded-tl-none shadow-sm border border-gray-100 text-sm text-gray-700 max-w-[85%]">
                    Hi! I'm your AI assistant. Look at the bottom right, that's
                    me!
                  </div>
                </div>
              </div>
              ...
              <div className="p-3 bg-white border-t border-gray-100">
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-gray-100 text-sm py-3 pl-4 pr-10 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all"
                  />
        ...
        <motion.button
          ...
          className={`pointer-events-auto flex items-center justify-center transition-all duration-300 rounded-full ${
            isChatOpen
              ? "w-14 h-14 bg-gray-800 text-white shadow-lg"
              : "w-36 h-36 md:w-40 md:h-40 bg-transparent"
          }`}
        >
